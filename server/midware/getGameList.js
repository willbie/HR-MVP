...
const getSteamAppList = (req, res) => {
  Steam.Steam.find({}, {_id:0, name:1, appid:1})
  .then((result) => {
    // var data = [];
      ...
      .then((response) => {
        if (response.data) {
          ...
          if (data.data !== undefined) {
            ...
            Steam.Steam.updateOne({"appid":id}, {"$set" : {"header_image":data.data.header_image}})
            .catch((err) => {
              console.log(err);
            })
          } else {
            res.send(invalidImage);
            Steam.Steam.updateOne({"appid":id}, {$set : {"header_image":invalidImage}})
            .catch((err) => {
              console.log(err);
            })
          }
